...
export class CanvasElementComponent implements OnInit {
  ...
  constructor(private service : ShapeListService, private fnMenuService : ShapeFnServiceService,
              private colorService : ColorService, private backFrontLinkService: BackFrontLinkService,
              private settingsMenuServiceService : SettingsMenuServiceService) { }
  ...
  ngOnInit(): void {
    ...
    this.settingsMenuServiceService.action$.subscribe(
      action => {
        this.setting = action;
      }
    );
    this.settingsMenuServiceService.shapes$.subscribe(
      shapes => {
        this.drawnShapes = shapes;
       // console.log(this.drawnShapes);
        const canvas = <HTMLCanvasElement> document.getElementById('canvasElement');
        let context = canvas.getContext('2d');
        context!.clearRect(0, 0, canvas.width, canvas.height);
        for(let i = this.drawnShapes.length - 1; i > -1; i--){
          console.log(this.drawnShapes[i]);
          context!.strokeStyle = this.drawnShapes[i].color;
          if(this.drawnShapes[i].name == 'rectangle'){
            if(this.drawnShapes[i].backgroundColor != '#FFFFFF'){
              context!.fillStyle = this.drawnShapes[i].backgroundColor;
  ...
  setPos(eClick: any){
    ...
    if(this.posX == -1 && this.posY == -1){
      this.posX = eClick.offsetX;
      this.posY = eClick.offsetY;
      this.newPosX = this.posX;
      this.newPosY = this.posY;
      if(this.option == 'cursor'){
        menu!.style.display = 'block';
        this.backFrontLinkService.findShape({x: this.posX,y: this.posY}).subscribe(
          shape=> {
            this.shapeGot = shape;
            this.fnMenuService.sendShape(this.shapeGot);
            this.fnMenuService.sendCoord({x: this.posX,y: this.posY});
          });
      }
    }
    else{
      ...
      if(this.option == 'rectangle'){
        const rectangle = {name: this.option, color: this.color, backgroundColor: this.backColor, corner:
            {x: this.posX, y: this.posY}, w: this.newPosX - this.posX, l: this.newPosY - this.posY};
        this.backFrontLinkService.addShape(rectangle).subscribe(() => {console.log("Done");});
      }
      else if(this.option == 'circle'){
        const circle = {name: this.option, color: this.color, backgroundColor: this.backColor, center:
            {x: this.posX, y: this.posY}, r: this.radius};
        this.backFrontLinkService.addShape(circle).subscribe(() => {console.log("Done");});
        this.radius = 0;
      }
      else if(this.option == 'ellipse'){
        const ellipse = {name: this.option, color: this.color, backgroundColor: this.backColor, center:
            {x: this.posX, y: this.posY}, r1: this.posX3, r2: this.posY3};
        this.backFrontLinkService.addShape(ellipse).subscribe(() => {console.log("Done");});
        this.posX3 = -1;
        this.posY3 = -1;
      }
      else if(this.option == 'triangle' || this.option == 'rightTri'){
        const triangle = {name: this.option, color: this.color, backgroundColor: this.backColor, v1:
            {x:this.posX, y: this.posY}, v2: {x: this.newPosX, y: this.newPosY}, v3: {x: this.posX3,y: this.posY3}};
        this.backFrontLinkService.addShape(triangle).subscribe(() => {console.log("Done");});
        this.posX3 = -1;
        this.posY3 = -1;
      }
      else if(this.option == 'line'){
        const line = {name: this.option, color: this.color, backgroundColor: this.backColor, start:
            {x:this.posX, y: this.posY}, end: {x: this.newPosX, y: this.newPosY}};
        this.backFrontLinkService.addShape(line).subscribe(() => {console.log("Done");});
      }
        ...
        if(this.shapeFn == 'move' || this.shapeFn == 'copy' || this.shapeFn == 'scale' || this.shapeFn == 'fill'){
          ...
          if(this.shapeGot.name == 'rectangle'){
            const rectangle = {name: this.shapeGot.name, color: this.shapeGot.color, backgroundColor: this.shapeGot.backgroundColor, corner:
                {x: this.shapeGot.corner.x, y: this.shapeGot.corner.y}, w: this.shapeGot.w, l: this.shapeGot.l};
            this.backFrontLinkService.addShape(rectangle).subscribe(() => {console.log("Done");});
          }
          else if(this.shapeGot.name == 'circle'){
            const circle = {name: this.shapeGot.name, color: this.shapeGot.color, backgroundColor: this.shapeGot.backgroundColor, center:
                {x: this.shapeGot.center.x, y: this.shapeGot.center.y}, r: this.shapeGot.r};
            this.backFrontLinkService.addShape(circle).subscribe(() => {console.log("Done");});
            this.radius = 0;
          }
          else if(this.shapeGot.name == 'ellipse'){
            const ellipse = {name: this.shapeGot.name, color: this.shapeGot.color, backgroundColor: this.shapeGot.backgroundColor, center:
            ...
            const triangle = {name: this.shapeGot.name, color: this.shapeGot.color, backgroundColor: this.shapeGot.backgroundColor, v1:
            ...
            const line = {name: this.shapeGot.name, color: this.shapeGot.color, backgroundColor: this.shapeGot.backgroundColor, start:
  ...
  draw(eDrag : any){
    ...
    this.newPosX = eDrag.offsetX;
    this.newPosY = eDrag.offsetY;
    ...
    if(this.option == 'cursor'){
      ...
      if(this.shapeFn == "move" || this.shapeFn == 'copy'){
        if(this.shapeGot.name == 'rectangle'){
          ...
          if(this.shapeGot.backgroundColor == '#FFFFFF'){
            ...
            context!.fillStyle = this.shapeGot.backgroundColor;
    ...
    if(this.option == "rectangle" || (this.option == 'cursor' && this.shapeFn == 'scale' && this.shapeGot.name == 'rectangle')){
      ...
      if(this.option == 'cursor' && this.shapeGot.backgroundColor != '#FFFFFF'){
        context!.fillStyle = this.shapeGot.backgroundColor;
